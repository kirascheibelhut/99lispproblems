...
46. (**) Truth tables for logical expressions.
         ...
         Example: table(A,B,and(A,or(A,B))).
                  true true true
                  true fail true
                  fail true fail
                  fail fail fail
...
47. (*) Truth tables for logical expressions (2).
        ...
        Example: table(A,B, A and (A or not B)).
                 true true true
                 true fail true
                 fail true fail
                 fail fail fail
...
In Lisp we represent the empty tree by 'nil' and the non-empty tree by the list (X L R), where X denotes the root node and L and R denote the left and right subtree, respectively. 
...
They are given as test cases in p54.lisp.
|#
...
54.(A)(*) Check whether a given term represents a binary tree
          ...
          Example: (istree (a (b nil nil) nil))
                   T
                   (istree (a (b nil nil)))
                   NIL
|#
...
57. (**) Binary search trees (dictionaries)
         ...
         Then use this predicate to test the solution of the problem 56.
         Example: test-symmetric([5,3,18,1,4,12,21]).
                  Yes
...
60. (**) Construct height-balanced binary trees with a given number of nodes
         ...
         Try to find a recursive statement and turn it into a predicate minNodes/2 defined as follows:
...
61. (*) Count the leaves of a binary tree
        ...
        Write a predicate count-leaves/2 to count them. 
...
63. (**) Construct a complete binary tree
         ...
         Write a predicate complete-binary-tree/2 with the following specification: 
...
65. (**) Layout a binary tree (2)
         ...
         Use the same conventions as in problem 64 and test your predicate in an appropriate way. 
|#
...
66. (***) Layout a binary tree (3)
          ...
          Use the same conventions as in problem 64 and 65 and test your predicate in an appropriate way. 
...
67. (**) A string representation of binary trees
         ...
         a) Write a Prolog predicate which generates this string representation, if the tree is given as usual (as nil or t(X,L,R) term). 
            Then write a predicate which does this inverse; 
            i.e. given the string representation, construct the tree in the usual form. 
            Finally, combine the two predicates in a single predicate tree-string/2 which can be used in both directions.
...
68. (**) Preorder and inorder sequences of binary trees
         ...
         a) Write predicates preorder/2 and inorder/2 that construct the preorder and inorder sequence of a given binary tree, respectively. 
            The results should be atoms, e.g. 'abdecfg' for the preorder sequence of the example in problem 67.
         ...
         c) If both the preorder sequence and the inorder sequence of the nodes of a binary tree are given, then the tree is determined unambiguously. 
            Write a predicate pre-in-tree/3 that does the job.

         d) Solve problems a) to c) using difference lists. 
            Cool! 
            Use the predefined predicate time/1 to compare the solutions.
...
69. (**) Dotstring representation of binary trees
         We consider again binary trees with nodes that are identified by single lower-case letters, as in the example of problem 67. 
         ...
         For example, the tree shown in problem 67 is represented as 'abd..e..c.fg...'. 
         First, try to establish a syntax (BNF or syntax diagrams) and then write a predicate tree-dotstring/2 which does the conversion in both directions. 
...
70.(B) (*) Check whether a given term represents a multiway tree
           ...
           Example: istree(t(a,[t(f,[t(g,[])]),t(c,[]),t(b,[t(d,[]),t(e,[])])])).
                    Yes
|#
...
71. (*) Determine the internal path length of a tree
        ...
        By this definition, the tree in the figure of problem 70 has an internal path length of 9. 
        Write a predicate ipl(Tree,IPL) for the flow pattern (+,-).
|#

#|
72. (*) Construct the bottom-up order sequence of the tree nodes
        Write a predicate bottom-up(Tree,Seq) which constructs the bottom-up sequence of the nodes of the multiway tree Tree. 
...
73. (**) Lisp-like tree representation
         ...
         Write a predicate tree-ltl(T,LTL) which constructs the "lispy token list" LTL if the tree is given as term T in the usual Prolog notation.
         Example: tree-ltl(t(a,[t(b,[]),t(c,[])]),LTL).
                  LTL = ['(', a, '(', b, c, ')', ')']
...
83. (**) Construct all spanning trees
         Write a predicate s-tree(Graph,Tree) to construct (by backtracking) all spanning trees of a given graph. 
         ...
         When you have a correct solution for the s-tree/2 predicate, use it to define two other useful predicates: is-tree(Graph) and is-connected(Graph). 
         Both are five-minutes tasks!
|#

#|
84. (**) Construct the minimal spanning tree
         Write a predicate ms-tree(Graph,Tree,Sum) to construct the minimal spanning tree of a given labelled graph. 
         Hint: Use the algorithm of Prim. 
               A small modification of the solution of 83 does the trick. 
...
92. (***) Von Koch's conjecture
          ...
          Given a tree with N nodes (and hence N-1 edges). 
...
#|
96. (**) Syntax checker (alternative solution with difference lists)
...
98. (***) Nonograms
          ...
          The "Sunday Telegraph" newspaper wrote: 
...
99. (***) Crossword puzzle
          ...
          Hints: (1) The problem is not easy. You will need some time to thoroughly understand it. 
                 ...
                 (2) Reading the data file is a tricky problem for which a solution is provided in the file p99-readfile.lisp. 
                 ...
                 (3) For efficiency reasons it is important, at least for larger puzzles, to sort the words and the sites in a particular order. 
                     For this part of the problem, the solution of 28 may be very helpful.
|#
